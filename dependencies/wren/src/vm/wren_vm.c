...
// CUSTOM PATCH!
void wrenGetMapKey(WrenVM* vm, int mapSlot, int* iterator, int elementSlot) 
{
  ...
  //uint32_t usedIndex = wrenValidateIndex(map->count, iterator);
  //ASSERT(usedIndex != UINT32_MAX, "Index out of bounds.");
  if (*iterator != 0) (*iterator)++;

  for (; *iterator < map->capacity; (*iterator)++)
  {
    MapEntry* entry = &map->entries[*iterator];
    if (!IS_UNDEFINED(entry->key)) {
      setSlot(vm, elementSlot, entry->key);
      return;
    }
  }
  *iterator = -1;
